...
if ($result = $connection->query("SELECT l.codigol AS codigol, l.isbn AS ISBN, l.titulo AS titulo, e.nombre AS Editorial, l.genero AS Genero, l.cantidad AS Stock, l.precio AS Precio, CONCAT(CONCAT(a.nombre, ' '), a.apellidos) AS Autor FROM (libro l JOIN editorial e ON l.codigoedi = e.codigoedi) JOIN autor a ON a.codigo = l.autor where Genero LIKE '%novela%'")) {
